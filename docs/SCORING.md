...
真实模式开启 `PANDA_DATA_AUTO_VERIFY` 后，平台仍会在同局开始前锁定 PandaAI 参考快照，并将输出事实验真结果作为独立审计信息保存。`dataVerification` 不会进入发送给 V1 评审模型的候选包或 Prompt，V1 评分器也不读取它来计算四维分或总分；它不会绕过模型竞技评分或充当评分失败兜底。

升级前已经保存的 V1 关键词规则评分记录不会自动迁移或重算，结果页明确标记为“历史规则评分”。如果操作者对这类记录显式执行“重跑这一局”，缺失的旧评分配置会按 DeepSeek 单模型缺省值处理，并用本次完整 CASE 模型评分替换被重跑 CASE 的旧快照；这不是后台批量迁移，也不表示新评测仍可使用规则评分。
