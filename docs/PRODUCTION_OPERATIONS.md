...
Manage the app only through systemd; do not start an additional Node process. The public browser entry is V1-only: Nginx forwards only its explicit V1 static/API allowlist, including long-running evaluation and SSE routes, to the loopback-only Node service. The shared public create/list/detail/events API still supports direct V2/history records, but the homepage must not advertise V2 or demo intake. Admin, appeal, evidence, and full operations are loopback/SSH-only. Public Nginx rejects unmatched and those private route families; use the SSH tunnel below for the complete loopback application when administering retained V2 records.
...
sudo systemd-run --quiet --wait --pipe --collect \
  ...
  'const submitted=process.env.V1_SUBMITTED_AGENT_TIMEOUT_MS; const runtime=process.env.LOCAL_RUNTIME_TIMEOUT_MS; console.log(JSON.stringify({v1SubmittedAgentTimeoutMs:Number(submitted),localRuntimeTimeoutMs:Number(runtime)})); if(submitted!=="7200000"||runtime!=="1800000") process.exit(1)'
```

Expected metadata is `root:root 600` and the non-secret timeout check prints `{"v1SubmittedAgentTimeoutMs":7200000,"localRuntimeTimeoutMs":1800000}`. The browser diagnostics retrieval copy is `/root/agent-review-access-key.txt`, also `root:root` mode `0600`. Authorized operators retrieve it only through their approved privileged-access procedure.
