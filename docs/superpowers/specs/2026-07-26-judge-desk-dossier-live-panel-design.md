...
Make the open human-review desk usable: reviewers can see redacted materials for both absolute and replica tracks while scoring. Stop silent V2 demo model panels when gateway credentials exist—or fail loudly when they do not—so leaf scores come from real model reviews.
...
6. **Live panel auto-config** — Extend `configuredReviewPanel`:
   1. If `MODEL_REVIEW_PANEL_JSON` set → parse as today (`mode: 'live'`).
   2. Else if `OPENAI_BASE_URL`+`OPENAI_API_KEY`, `ARK_BASE_URL`+`ARK_API_KEY`, and four `REVIEW_MODEL_*` (OpenAI / Anthropic / Doubao / DeepSeek) are usable → build a live panel: four primaries with the same gateway mapping as `configuredReviewers` (gpt/claude → LLMX; doubao/deepseek → Ark), plus arbitrator `{ id: 'arbitrator', … }` on Ark using `REVIEW_MODEL_DEEPSEEK` (distinct `identityKey` from the DeepSeek primary).
   3. Else if `MODEL_REVIEW_PANEL_MODE=demo` (or equivalent test injection) → allow deterministic demo panel.
   4. Else → **throw** with an explicit message listing missing credentials/models. No silent demo default.
7. **Create-time failure** — V2 evaluation create / Phase-2 panel construction surfaces the same error so operators cannot start a “live-looking” run on mock seats.
...
- Default production path uses live panel from gateway env or explicit `MODEL_REVIEW_PANEL_JSON`; silent demo is gone
