...
const state = { evaluationId: location.hash.slice(1), token: '' };
...
async function loadConcreteTargets() {
  ...
  const tests = item.runtimeState?.runIndex || [];
  const evidence = item.evidenceManifest?.items || [];
  const options = [
    '<option value="">选择具体路径</option>',
    ...tests.map((cell, index) => option('test', cell.identity?.testId, `runtimeState.runIndex[${index}]`, `测试：${cell.identity?.testId}`)),
    ...evidence.map((item, index) => option('evidence', item.evidenceId, `evidenceManifest.items[${index}]`, `证据：${item.evidenceId}`)),
    option('score', 'absolute', 'resultV2.absolute', '绝对分')
  ];
...
function renderAppeal(appeal) {
  ...
  return `<article><header><small>APPEAL ${escapeHtml(appeal.appealId)}</small><b>${escapeHtml(appeal.status)}</b></header>
    ...
    <p class="comparison">原锁定结果：${escapeHtml(before)}<br>后续版本仅会在受控重算完成后追加，原始运行不覆盖。</p>
