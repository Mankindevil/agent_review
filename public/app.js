...
function renderBuilds(builds, item) {
  ...
  if (activity && !builds.some((build) => activityMatches(activity, build.runtimeId))) {
    cards.push(`<article class="build-card build-card-loading work-active" data-skill-runtime="${escapeHtml(activity.key || '')}"><div class="build-row"><b>${escapeHtml(activity.target || 'Runtime')}</b><span>${activityPosition(activity)}</span><code>公开基线输入已封舱</code><span class="work-status">生成中</span>${renderWorkLoader(activity, 'row')}</div></article>`);
  }
